...
degree=1:10
ind = sample(nrow(Auto)) # shuflle the data around- then split in half with ind[1:nrow(Auto)/2] to get testing data set
for(d in degree){
...
plot(degree,val.error10,type="b", ylim = c(15,30))
...
# IDEA.. the problem with just using a single split (one training data set and one testing data set) is that 
# we get vary different values of MSE (for the same value of the parameter) because of the variability in our sample.
# i.e. the MSE for the testing data set will be highly dependent on which data is sampled.  
...
plot(degree,cv.error10,type="b", ylim = c(15,30))
...
#### NOTE that the variation in MSE for each of the levels of the parameter (in this case the degree of the
# polynomial being fit in the model) is much smaller for cross validation than it was for the validation set


## 5-fold CV (to select tuning parameter)
cv.error5=rep(0,10)
for(d in degree){
  set.seed(1) # note the setting of the seed here
  glm.fit=glm(mpg~poly(horsepower,d), data=Auto)
