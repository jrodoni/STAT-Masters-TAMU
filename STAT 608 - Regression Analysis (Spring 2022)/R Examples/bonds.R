bonds <- read.csv("C:/Users/jackr/OneDrive/Desktop/Graduate School Courses/STAT 608 - Regression Analysis/Data/bonds.csv")
...
hii<-hatvalues(my.lm)  #The leverages: only gives the on-diagonals of the hat matrix.
...
#Match Cook's D for 13th point.
y_hat <- fitted(my.lm)
y_hat_j_13 <- drop(cbind(1, bonds$CouponRate) %*% coef(m3))
sg_hat <- summary(my.lm)$sigma
D_13 <- sum((y_hat_j_13 - y_hat) ^ 2) / (2 * sg_hat ^ 2)
...
#Match Cook's D for 1st point.
y_hat_j_1 <- drop(cbind(1, bonds$CouponRate) %*% coef(m3))
D_1 <- sum((y_hat_j_1 - y_hat) ^ 2) / (2 * sg_hat ^ 2)
