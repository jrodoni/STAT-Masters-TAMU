...
nothing<-lm(log(Time) ~ 1, data = bridge)
addterm(nothing, scope=full, test="F")

m1<-lm(log(Time)~log(Dwgs), data = bridge)
...
###########################################################
#Stepwise:  Use alpha to enter = 0.05, alpha to remove = 0.05
nothing<-lm(log(Time)~1, data = bridge)
...
m2<-lm(log(Time)~log(Dwgs) + log(Spans), data = bridge)  #Backward Step?
summary(m2)
...
m3 <- lm(log(Time)~log(Dwgs) + log(Spans) + log(CCost), data = bridge)
...
## Using adjusted R^2 as the criterion, our optimal model has LDwgs, LSpans, and LCCost.
...
fit_glmnet <- glmnet(zX, log(Time), alpha = 1, standardize = FALSE)

## This shows the values of the penalty threshold used at each step.
fit_glmnet$lambda

## This shows the beta coefficients for each penalty threshold. I do not know
## how exactly these relate to the lambda values. Can't tell from the
## documentation.
fit_glmnet$beta

## Using cross-validation to choose the optimal penalty threshold.
cv.out <- cv.glmnet(zX, log(Time), alpha = 1)
plot(cv.out)
cv.out$lambda.min
