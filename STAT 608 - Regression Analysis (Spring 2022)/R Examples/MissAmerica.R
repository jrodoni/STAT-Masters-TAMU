####
#### Data on the Miss America beauty pageant. The response variable is the
#### number of times each US state (and DC) has produced top ten finalists for
#### the years 2000-2008. We have variables for longitude and latitude (it is
#### expected that southern states place more importance on beauty pageants) as
#### well as the total land and water area (with the expectation that search is
#### more difficult over larger geographic areas).
####

MA<-read.csv("MissAmerica08.csv", header=TRUE)
...
# Latitude is north-south position, while longitude is east-west.
...
MAYr<- read.csv("MissAmerica08Yrs.csv", header=TRUE)
attach(MAYr)
y<-Top10
